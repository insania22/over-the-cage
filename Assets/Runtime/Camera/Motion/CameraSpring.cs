...
public class CameraSpring : MonoBehaviour
{
    ...
    private static void Spring(ref Vector3 current, ref Vector3 velocity, Vector3 target, float halflife, float frequency, float dt)
    {
        var dampingRatio = -Mathf.Log(0.5f) / (frequency * halflife);
        var f = 1.0f + 2.0f * dt * dampingRatio * frequency;
        var oo = frequency * frequency;
        var hoo = dt * oo;
        var hhoo = dt * hoo;
        var detInv = 1.0f / (f + hhoo);
        var detX = f * current + dt * velocity + hhoo * target;
